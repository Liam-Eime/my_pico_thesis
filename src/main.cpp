...
int main() {
    ...
    // Scratch buffer for the demodulated envelope
    std::vector<float> envelope;
    envelope.reserve(1024); // heuristic; will grow as needed

    // Main loop: on each ready ADC buffer, demodulate + filter, then let the logger handle triggers
    ...
    while (true) {
        ...
        if (adc_sampler_take_ready(&ready_buf, &count)) {
            // Demodulate the 20 kHz carrier at 25% duty and filter at the envelope rate
            envelope.clear();
